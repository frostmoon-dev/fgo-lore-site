...
export async function relay(request, path) {
  ...
  try {
    ...
    const reason = timeout ? "it took too long to answer" : err.cause?.code ?? err.message;
    // Marked as the relay's own, so the browser does not retry a bad base URL.
    return json({ error: `Could not reach the API (${reason}). Check the base URL.` }, timeout ? 504 : 502, { "X-Relay-Error": "1" });
  }
  ...
  return new Response(upstream.body, {
    ...
    headers: {
      ...
      // How long a busy provider asks to wait before trying again.
